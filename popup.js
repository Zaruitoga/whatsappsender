...
document.getElementById("clearLogsBtn").addEventListener("click", () => {
    chrome.storage.local.remove("logs", () => {
        if (chrome.runtime.lastError) {
            ...
            document.getElementById("logContainer").innerHTML = ""; // Vide l'affichage des logs
        }
    });
});
...
// Récupération des données dans le chrome storage lorsque le popup est ouvert
...
// Chargement des logs au démarrage
function loadLogs() {
    loadData("logs", (logs) => {
        if (!logs || !Array.isArray(logs)) logs = [];
        ...
        logs.forEach(log => {
            ...
            // Vérifie si le log est de type debug et applique la visibilité en fonction de debugToggle
            if (log.type === "debug") {
